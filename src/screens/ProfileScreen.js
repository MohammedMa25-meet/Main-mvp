...
// Import Firebase services to save data
import { db } from '../services/firebase';
import { doc, updateDoc } from 'firebase/firestore';
// Import your new Appwrite service for file uploads
...
const ProfileScreen = ({ navigation, onScreenChange }) => {
  ...
  // This state will hold the temporary data while editing in the modal
  const [editData, setEditData] = useState({});

  // Get user's full name from the context
  ...
  const pickImage = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        const fileName = file.uri.split('/').pop() || 'profile-image.jpg'; // Create a name for the file

        Alert.alert(t('Uploading...'), t('Your new profile picture is being uploaded.'));

        try {
          // 1. Upload the new profile image to Appwrite
          const imageUrl = await uploadFile({ uri: file.uri, name: fileName }, userData.uid);

          // 2. Update the user's profile in Firestore with the new image URL
          const userDocRef = doc(db, 'users', userData.uid);
          await updateDoc(userDocRef, {
            profileImage: imageUrl,
          });

          // 3. Update the global context to show the new image instantly
          ...
          // Fallback: Store the local URI temporarily
          const userDocRef = doc(db, 'users', userData.uid);
          await updateDoc(userDocRef, {
            profileImage: file.uri, // Store local URI as fallback
          });

          // Update the global context
          updateUserData({ ...userData, profileImage: file.uri });

          Alert.alert(t('Success'), t('Profile image updated (local storage).'));
        }
      }
    } catch (error) {
    ...
    Alert.alert(
      ...
      [
        { text: t('Cancel'), style: 'cancel' },
        {
          text: t('Logout'),
          onPress: () => {
            // Here you would also call a function to sign the user out of Firebase Auth
  ...
  // When the "Edit" button is pressed, populate the modal with the current user data
  const handleEdit = () => {
    setEditData({
  ...
  // When changes are saved, update Firebase and the local context
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      // Update the data in our global context to reflect the changes instantly
      updateUserData({
        ...userData,
        firstName: editData.firstName,
        lastName: editData.lastName,
  ...
  const handleRetakeQuestionnaire = () => {
    // Clear any existing questionnaire answers to start fresh
    ...
    // Update the user context with cleared data
    updateUserData(clearedUserData);
    
    // Navigate to questionnaire with cleared data
  ...
  return (
    <SafeAreaView style={containerStyle}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.card, cardStyle]}>
          ...
          <View style={styles.detailsGrid}>
            <View style={styles.detailItem}><Text style={[styles.detailLabel, textStyle]}>{t('Full Name')}</Text><Text style={[styles.detailValue, titleStyle]}>{fullName}</Text></View>
            <View style={styles.detailItem}><Text style={[styles.detailLabel, textStyle]}>{t('Email')}</Text><Text style={[styles.detailValue, titleStyle]}>{userData.email || t('Not specified')}</Text></View>
            <View style={styles.detailItem}><Text style={[styles.detailLabel, textStyle]}>{t('Phone')}</Text><Text style={[styles.detailValue, titleStyle]}>{userData.phone || t('Not specified')}</Text></View>
            <View style={styles.detailItem}><Text style={[styles.detailLabel, textStyle]}>{t('Age')}</Text><Text style={[styles.detailValue, titleStyle]}>{userData.age || t('Not specified')}</Text></View>
          </View>
        </View>
        ...
        <View style={[styles.card, cardStyle]}>
          ...
          <View style={styles.questionnaireSection}>
            <View style={styles.questionItem}><Text style={[styles.questionLabel, titleStyle]}>{t('Career Goal')}</Text><Text style={[styles.questionAnswer, textStyle]}>{userData.careerGoal || t('Not specified')}</Text></View>
            <View style={styles.questionItem}><Text style={[styles.questionLabel, titleStyle]}>{t('Region')}</Text><Text style={[styles.questionAnswer, textStyle]}>{userData.region || t('Not specified')}</Text></View>
            <View style={styles.questionItem}><Text style={[styles.questionLabel, titleStyle]}>{t('Experience Level')}</Text><Text style={[styles.questionAnswer, textStyle]}>{userData.experience || t('Not specified')}</Text></View>
            <View style={styles.questionItem}>
              <Text style={[styles.questionLabel, titleStyle]}>{t('Field of Experience')}</Text>
              <View style={styles.answerTags}>
                {userData.field && userData.field.length > 0 ? (
                  userData.field.map((field, index) => (
                    <View key={index} style={styles.answerTag}><Text style={styles.answerTagText}>{field}</Text></View>
                  ))
              ...
              <View style={styles.answerTags}>
                {userData.languages && userData.languages.length > 0 ? (
                  userData.languages.map((language, index) => (
                    <View key={index} style={styles.answerTag}><Text style={styles.answerTagText}>{language}</Text></View>
                  ))
                ) : <Text style={[styles.questionAnswer, textStyle]}>{t('Not specified')}</Text>}
              </View>
            </View>
      ...
      {/* Edit Profile Modal */}
      <Modal visible={showEditModal} animationType="slide" presentationStyle="pageSheet" transparent={false}>
        <SafeAreaView style={[styles.modalContainer, isDarkMode && styles.modalContainerDark]}>
          <View style={[styles.modalHeader, isDarkMode && styles.modalHeaderDark]}>
            ...
            <TouchableOpacity onPress={handleCancelEdit}><Ionicons name="close" size={24} color={isDarkMode ? "#FFFFFF" : "#1F2937"} /></TouchableOpacity>
          </View>
          <ScrollView style={styles.modalContent}>
            <View style={styles.inputGroup}>
              <Text style={[styles.inputLabel, textStyle]}>{t('First Name')}</Text>
              <TextInput style={[styles.textInput, isDarkMode && styles.textInputDark]} value={editData.firstName} onChangeText={(text) => setEditData({ ...editData, firstName: text })} />
            </View>
            <View style={styles.inputGroup}>
              <Text style={[styles.inputLabel, textStyle]}>{t('Last Name')}</Text>
              <TextInput style={[styles.textInput, isDarkMode && styles.textInputDark]} value={editData.lastName} onChangeText={(text) => setEditData({ ...editData, lastName: text })} />
            </View>
            <View style={styles.inputGroup}>
              <Text style={[styles.inputLabel, textStyle]}>{t('Phone')}</Text>
              <TextInput style={[styles.textInput, isDarkMode && styles.textInputDark]} value={editData.phone} onChangeText={(text) => setEditData({ ...editData, phone: text })} keyboardType="phone-pad" />
            </View>
          </ScrollView>
          <View style={[styles.modalFooter, isDarkMode && styles.modalFooterDark]}>
            <TouchableOpacity style={styles.cancelButton} onPress={handleCancelEdit}><Text style={styles.cancelButtonText}>{t('Cancel')}</Text></TouchableOpacity>
            <TouchableOpacity style={styles.saveButton} onPress={handleSaveEdit} disabled={isSaving}>
              {isSaving ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.saveButtonText}>{t('Save Changes')}</Text>}
            </TouchableOpacity>
          </View>
        </SafeAreaView>
      </Modal>
